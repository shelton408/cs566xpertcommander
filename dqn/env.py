...
class Env(object):
    ...
    def run(self, is_training=False):
        ...
        while not self.is_over():
            ...
            # Eval playable cards
            prev_playable_cards = self.get_playable_cards()

            # Environment steps
            next_state, next_player_id = self.step(action, self.agents[player_id].use_raw)

            # Eval playable cards after take action
            playable_cards = self.get_playable_cards()

            if prev_playable_cards - playable_cards < 5:
                self.game.payoffs[0].append(1)
            else:
                self.game.payoffs[0].append(-1)
    ...
    def get_played_cards(self):
        ''' Get current playable cards. Must be implemented in the child class.

        Returns:
            (int): Number of playable cards.
